        ...
        Credentials({
            ...
            authorize: async (credentials) => {
                ...
                return {
                    ...
                    profileImg: user.profileImg || DEFAULT_AVATAR
                }
            },
        }),
    ],
    callbacks: {
        ...
        async jwt({ token, user, trigger, session, account }) {
            ...
            if (user && account === null) {
                ...
                token.profileImg = user.profileImg || token.profileImg || DEFAULT_AVATAR;
            }
            ...
            if (user && (account?.provider === "github" || account?.provider === "google")) {
                try {
                    ...
                    if (dbUser) {
                        ...
                        token.profileImg = dbUser.profileImg || user.image || DEFAULT_AVATAR;
                    }
                } catch (error) {
        ...
        async session({ session, token }) {
            if (token) {
                ...
                session.user.profileImg = token.profileImg ?? DEFAULT_AVATAR;
            }
            return session;
        },
    },
